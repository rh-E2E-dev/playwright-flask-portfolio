...
test.describe('ログイン', () => {
  ...
  test.describe('動作（正常系）', () => {
    test('ログインできること', async ({ page }) => {
      await page.goto(`${process.env.BASE_URL!}/login`);
      await page.getByRole('textbox', { name: 'ユーザー名' }).fill(process.env.NAME_user1!);
      await page.getByRole('textbox', { name: 'パスワード' }).fill(process.env.PASSWORD_user1!);
    ...
    test.describe('不正', () => {
      test('ユーザー名、パスワード共に不正でログインエラーが表示されること', async ({ page }) => {
        ...
        await page
          ...
          .fill(`${process.env.NAME_user1!}wrong`);
        await page
          .getByRole('textbox', { name: 'パスワード' })
          .fill(`${process.env.PASSWORD_user1}wrong`);
        await page.getByRole('button', { name: 'ログイン' }).click();
        await expect(page.getByText(/ログインに失敗しました/)).toBeVisible();
      });
      test('ユーザー名のみ不正でログインエラーが表示されること', async ({ page }) => {
        await page.goto(`${process.env.BASE_URL!}/login`);
        await page.getByRole('textbox', { name: 'ユーザー名' }).fill(process.env.NAME_user2!);
        await page.getByRole('textbox', { name: 'パスワード' }).fill(process.env.PASSWORD_user1!);
        await page.getByRole('button', { name: 'ログイン' }).click();
        await expect(page.getByText(/ログインに失敗しました/)).toBeVisible();
      });
      test('パスワードのみ不正でログインエラーが表示されること', async ({ page }) => {
        await page.goto(`${process.env.BASE_URL!}/login`);
        await page.getByRole('textbox', { name: 'ユーザー名' }).fill(process.env.NAME_user1!);
        await page.getByRole('textbox', { name: 'パスワード' }).fill(process.env.PASSWORD_user2!);
        await page.getByRole('button', { name: 'ログイン' }).click();
        await expect(page.getByText(/ログインに失敗しました/)).toBeVisible();
      });
    });
